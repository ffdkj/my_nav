...
sudo curl -fsSLO https://raw.githubusercontent.com/ffdkj/my_nav/v0.2.1/deploy/compose.yaml
sudo curl -fsSL  https://raw.githubusercontent.com/ffdkj/my_nav/v0.2.1/deploy/.env.example -o .env
...
deploy/wait-image.sh 0.2.1     # READY 之后再去主机上拉，否则会拉到 MANIFEST_UNKNOWN

cd /opt/1panel/docker/compose/my_nav
# 注意：这台主机上这个 1Panel compose 项目的文件名是 docker-compose.yml（仓库里叫 compose.yaml）
sudo sed -i 's|^    image: .*|    image: ghcr.io/ffdkj/my_nav:0.2.1|' docker-compose.yml
...
| 症状 | 原因 / 处理 |
|---|---|
| `manifest unknown` | 镜像标签写错。可用标签：`0.2.1`、`latest`、`edge`（`edge` 跟 main 分支） |
